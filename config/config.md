...
| `wal.flush_interval` | String | `1s` | The interval of flushing buffered entries to the object store, at least `10ms`, defaults to `1s`.<br/>Every interval with pending entries costs one object request, and the durable acknowledgement latency of a write is bounded below by the interval.<br/>**It's only used when the provider is `experimental_object_store`**. |
