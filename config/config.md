...
| `wal.prefix` | String | `wal` | The path prefix of the WAL objects inside the storage provider.<br/>**It's only used when the provider is `experimental_object_store`**. |
| `wal.flush_interval` | String | `1s` | The interval of flushing buffered entries to the object store, at least `10ms`, defaults to `1s`.<br/>Each non-empty timer-triggered flush creates one object; a batch that reaches `max_batch_bytes` is flushed immediately, and a failed create adds a read-back.<br/>An append waits for the next tick plus the create, so a smaller interval lowers acknowledgement latency at the cost of more object requests when writes are sparse.<br/>**It's only used when the provider is `experimental_object_store`**. |
