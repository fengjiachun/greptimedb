...
- *Proposed* garbage collection derives its watermark from each region's manifest `flushed_entry_id`, deletes an object only when every segment's maximum entry id is at or below its region's watermark, and always keeps the highest-sequence object. The handoff already exists: Mito hands that value to the store on region open, and the proposal introduces no additional persisted state.
