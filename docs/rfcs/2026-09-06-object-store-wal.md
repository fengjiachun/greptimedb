...
- Recovery that never silently loses data. Today structural corruption fails recovery, and a corrupted segment fails the read or replay that decodes it; nothing is skipped. *Proposed*: a corrupted segment is skipped at segment granularity only when configured to, and the affected region is marked and counted.
...
Entry ids are assigned per region and are contiguous: when a batch admits entries of a region, each entry receives the region's previous id plus one, independently of the object sequence. Allocation therefore never skips an id, but the recovered history of a region may contain gaps where a batch was never acknowledged, as described under *Writing*, and the object sequence cannot be derived from an entry id without the catalog.
...
*Proposed* addition, not accepted by the current configuration:
