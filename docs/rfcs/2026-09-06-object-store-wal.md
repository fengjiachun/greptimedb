...
# sparse `durable` writes and raises the request count, at most one
# object per interval per node; under sustained load the batch seals on
# size and the interval no longer matters.
...
- `LogStore::wait_durable` has a default implementation that returns at once, so Raft Engine, Kafka and the noop provider are unchanged by the durability barrier.
...
The default interval is 100 ms because it removes most of the seal-timer latency of a sparse `durable` writer, 52 ms of residual wait against 747 ms at 1 s, at a request cost that stays bounded: when writes are sparse the store creates at most one object per interval per node, ten per second at the default, and under sustained load the batch seals on size and the interval no longer matters. Going to 10 ms buys another 40 ms for ten times the request ceiling. The default acknowledgement mode stays `durable` because that is what a write-ahead log means: an acknowledged write survives a crash. `enqueued` is an explicit choice with a documented loss bound, the unpersisted backlog, and the crash gate reports what it loses.
...
- Every object costs a request. When writes are sparse the store creates at most one object per interval per node, ten per second at the default; under sustained load the count is governed by aggregate bytes and `max_batch_bytes`. The defaults were calibrated on a local object store, not on a cloud bill.
